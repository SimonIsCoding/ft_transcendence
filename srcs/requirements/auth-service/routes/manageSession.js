...
export async function sendSessionIdRoute(fastify)
{
...
export async function activeSessionRoute(fastify)
{
	fastify.post('/checkSession', async (request, reply) => {
		const { currentUser } = request.body;
		const stmt = db.prepare(`SELECT * FROM sessions WHERE user_id = ? and connected = 1`);
...
export async function deleteSessionRoute(fastify)
{
