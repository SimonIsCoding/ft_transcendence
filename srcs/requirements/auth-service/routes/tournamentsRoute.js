...
export async function tournamentsRoutes(fastify) {
    ...
    fastify.put('/game/tournaments/:id', { preHandler: fastify.auth }, async (req, reply) => {
        try {
            ...
            const currentTournament = db.prepare(
                `SELECT * FROM tournaments WHERE tournamentid = ?`
            ).get(tournamentId);
    ...
    fastify.post('/game/tournaments', { preHandler: fastify.auth }, async (req, reply) => {
        try {
            ...
            // Validate user_player is valid (0-4)
            if (user_player === undefined || user_player === null) {
                return reply.status(400).send({
                    error: 'Missing user_player',
                    message: 'user_player is required (0-4)'
                });
            }

            if (user_player < 0 || user_player > 4) {
                return reply.status(400).send({
            ...
            const result = db.prepare(
                `INSERT INTO tournaments 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?)`
            ).run(userId, player1_alias, player2_alias, player3_alias, player4_alias, user_player);
