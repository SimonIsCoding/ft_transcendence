var A=Object.defineProperty;var I=(s,t,e)=>t in s?A(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var a=(s,t,e)=>I(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const c of r.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function e(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=e(n);fetch(n.href,r)}})();const f="https://localhost:4443/api/auth",u=class u{constructor(){a(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const t=localStorage.getItem("user");return t?JSON.parse(t):null}async login(t,e,i){var n,r,c;try{const l=await fetch(`${f}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:e})}),d=await l.json();if(!l.ok||d.status!=="success")throw new Error(d.error||d.message||"Login failed");if(!((r=(n=d.data)==null?void 0:n.user)!=null&&r.email)||!((c=d.data)!=null&&c.token))throw new Error("Invalid server response format");const y={email:d.data.user.email,token:d.data.token,alias:i||d.data.user.alias||d.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(l){throw console.error("Login error:",l),new Error(l instanceof Error?l.message:"An unknown error occurred")}}async register(t,e,i){var n,r,c;try{const l=await fetch(`${f}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:e,...i&&{alias:i}})}),d=await l.json();if(!l.ok||d.status!=="success")throw new Error(d.error||d.message||"Registration failed");if(!((r=(n=d.data)==null?void 0:n.user)!=null&&r.email)||!((c=d.data)!=null&&c.token))throw new Error("Invalid server response format");const y={email:d.data.user.email,token:d.data.token,alias:i||d.data.user.alias||d.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(l){throw console.error("Registration error:",l),new Error(l instanceof Error?l.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(t){const e=this.getCurrentUser();e&&(e.alias=t,this.saveUser(e))}onAuthStateChanged(t){this.listeners.push(t)}saveUser(t){localStorage.setItem("user",JSON.stringify(t)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(t=>t())}};a(u,"instance");let v=u;const h=v.getInstance(),w={render(){const s=h.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const t=parseFloat(s.value);b.setBallSpeed(t)})}},T={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),b.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),b.handleKeyUp(s.key)})}};class H{constructor(){a(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const t=document.getElementById("auth-container");if(t){const e=h.getCurrentUser();t.innerHTML=e?w.renderProfile(e.email):w.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",t=>{const e=t.target;e.closest("#login-btn")?(this.isLoginView=!0,this.render()):e.closest("#register-btn")?(this.isLoginView=!1,this.render()):e.closest("#logout-btn")?(h.logout(),this.isLoginView=!0,this.render()):e.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async t=>{t.target.id==="auth-form"&&(t.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const t=document.getElementById("email"),e=document.getElementById("password"),i=document.getElementById("auth-error");if(!t||!e||!i)return;const n=t.value,r=e.value;try{this.isLoginView?await h.login(n,r):await h.register(n,r),this.render()}catch(c){i.textContent=c.message}}}const M=new H,P={currentUser:null,isLogin:!0,render(){return`
    ...
    `},renderAuth(){const s=h.getCurrentUser();return s?w.renderProfile(s.email):w.renderAuthForm(this.isLogin)},init(){this.currentUser=h.getCurrentUser(),b.initGameCanvas(),T.init(),M.init(),E.initSettings(),h.onAuthStateChanged(()=>{this.currentUser=h.getCurrentUser(),document.getElementById("auth-container").innerHTML=this.renderAuth()})}};class _{constructor(){a(this,"currentUser",null);this.initializeUser()}initializeUser(){const t=localStorage.getItem("authToken"),e=localStorage.getItem("userEmail");t&&e?this.currentUser={email:e,token:t}:m.navigate("home")}render(){var t;return`
    ...
    `}initEventListeners(){const t=document.getElementById("logout-btn");t&&t.addEventListener("click",this.handleLogout.bind(this))}handleLogout(){localStorage.removeItem("authToken"),localStorage.removeItem("userEmail"),this.currentUser=null,m.navigate("home")}initialize(){const t=document.getElementById("app");t?(t.innerHTML=this.render(),this.initEventListeners()):console.error("App container not found")}}function S(){document.getElementById("login-btn").addEventListener("click",()=>{const t=document.getElementById("login").value,e=document.getElementById("password").value;fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:t,password:e})}).then(i=>i.json()).then(i=>{localStorage.setItem("login",t);const n=localStorage.getItem("login");n&&i.success==!0&&(document.getElementById("title").textContent=`Hi ${n}`),document.getElementById("welcome-div").style.display=i.success?"block":"none",document.getElementById("welcome-div").textContent=`Welcome ${n}, you are now connected :)`}),console.log(t,"Password:",e)})}function C(){const s=document.getElementById("registerBtn");s==null||s.addEventListener("click",()=>{var i;if(document.getElementById("connexionBlock").style.display="none",document.getElementById("register-container"))return;const e=document.createElement("div");e.id="register-container",e.innerHTML=`
	...
	`,document.body.appendChild(e),(i=document.getElementById("create-account"))==null||i.addEventListener("click",()=>{const n=document.getElementById("new-username").value,r=document.getElementById("new-password").value,c=document.getElementById("new-alias").value;fetch("/api/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:n,password:r,alias:c})}).then(l=>l.json()).then(l=>console.log("Account created:",l)).catch(l=>console.error(l))})})}function F(){document.addEventListener("DOMContentLoaded",()=>{var e,i;const s=document.getElementById("registrationBlock"),t=document.getElementById("connexionBlock");(e=document.getElementById("registerBtn"))==null||e.addEventListener("click",()=>{t.classList.add("hidden"),s.classList.remove("hidden")}),(i=document.getElementById("backToLogin"))==null||i.addEventListener("click",()=>{s.classList.add("hidden"),t.classList.remove("hidden")})})}class L{static render(){return`
	...
	</div>`}static init(){console.log("in webdev/pong/src/views/login.ts: Login page initialized"),S(),C()}}class m{static navigate(t){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",t==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(t){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=P.render(),P.init();break;case"dashboard":new _().initialize();break;case"login":this.app.innerHTML=L.render(),L.init();break}history.pushState({},"",t==="home"?"/":`/${t}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const t=window.location.pathname;this.navigate(t.includes("dashboard")?"dashboard":t.includes("login")?"login":"home")}),window.addEventListener("popstate",()=>{const t=window.location.pathname;this.navigate(t.includes("dashboard")?"dashboard":t.includes("login")?"login":"home")})}}a(m,"app",document.getElementById("app"));m.init();window.addEventListener("load",()=>{const s=window.location.pathname;m.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":"home"),S(),C(),F()});window.addEventListener("popstate",()=>{const s=window.location.pathname;m.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":"home")});
