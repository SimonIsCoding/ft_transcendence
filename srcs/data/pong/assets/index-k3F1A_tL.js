var B=Object.defineProperty;var k=(s,e,t)=>e in s?B(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var a=(s,e,t)=>k(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();const w="https://localhost:4443/api/auth",g=class g{constructor(){a(this,"listeners",[])}static getInstance(){return g.instance||(g.instance=new g),g.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,i){var n,o,d;try{const c=await fetch(`${w}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t})}),l=await c.json();if(!c.ok||l.status!=="success")throw new Error(l.error||l.message||"Login failed");if(!((o=(n=l.data)==null?void 0:n.user)!=null&&o.email)||!((d=l.data)!=null&&d.token))throw new Error("Invalid server response format");const h={email:l.data.user.email,token:l.data.token,alias:i||l.data.user.alias||l.data.user.email.split("@")[0]};return this.saveUser(h),h}catch(c){throw console.error("Login error:",c),new Error(c instanceof Error?c.message:"An unknown error occurred")}}async register(e,t,i){var n,o,d;try{const c=await fetch(`${w}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...i&&{alias:i}})}),l=await c.json();if(!c.ok||l.status!=="success")throw new Error(l.error||l.message||"Registration failed");if(!((o=(n=l.data)==null?void 0:n.user)!=null&&o.email)||!((d=l.data)!=null&&d.token))throw new Error("Invalid server response format");const h={email:l.data.user.email,token:l.data.token,alias:i||l.data.user.alias||l.data.user.email.split("@")[0]};return this.saveUser(h),h}catch(c){throw console.error("Registration error:",c),new Error(c instanceof Error?c.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};a(g,"instance");let v=g;const u=v.getInstance(),b={render(){const s=u.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const e=parseFloat(s.value);f.setBallSpeed(e)})}},M={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),f.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),f.handleKeyUp(s.key)})}};class _{constructor(){a(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const e=document.getElementById("auth-container");if(e){const t=u.getCurrentUser();e.innerHTML=t?b.renderProfile(t.email):b.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",e=>{const t=e.target;t.closest("#login-btn")?(this.isLoginView=!0,this.render()):t.closest("#register-btn")?(this.isLoginView=!1,this.render()):t.closest("#logout-btn")?(u.logout(),this.isLoginView=!0,this.render()):t.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async e=>{e.target.id==="auth-form"&&(e.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const e=document.getElementById("email"),t=document.getElementById("password"),i=document.getElementById("auth-error");if(!e||!t||!i)return;const n=e.value,o=t.value;try{this.isLoginView?await u.login(n,o):await u.register(n,o),this.render()}catch(d){i.textContent=d.message}}}const F=new _,P={currentUser:null,isLogin:!0,render(){return`
    ...
    `},renderAuth(){const s=u.getCurrentUser();return s?b.renderProfile(s.email):b.renderAuthForm(this.isLogin)},init(){this.currentUser=u.getCurrentUser(),f.initGameCanvas(),M.init(),F.init(),E.initSettings(),u.onAuthStateChanged(()=>{this.currentUser=u.getCurrentUser(),document.getElementById("auth-container").innerHTML=this.renderAuth()})}};class U{constructor(){a(this,"currentUser",null);this.initializeUser()}initializeUser(){const e=localStorage.getItem("authToken"),t=localStorage.getItem("userEmail");e&&t?this.currentUser={email:t,token:e}:y.navigate("home")}render(){var e;return`
    ...
    `}initEventListeners(){const e=document.getElementById("logout-btn");e&&e.addEventListener("click",this.handleLogout.bind(this))}handleLogout(){localStorage.removeItem("authToken"),localStorage.removeItem("userEmail"),this.currentUser=null,y.navigate("home")}initialize(){const e=document.getElementById("app");e?(e.innerHTML=this.render(),this.initEventListeners()):console.error("App container not found")}}function C(){document.getElementById("login-btn").addEventListener("click",()=>{const e=document.getElementById("login").value,t=document.getElementById("password").value;fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:e,password:t})}).then(i=>i.json()).then(i=>{localStorage.setItem("login",e);const n=localStorage.getItem("login");n&&i.success==!0?(document.getElementById("title").textContent=`Hi ${n}`,document.getElementById("welcome-div").style.display="block",document.getElementById("welcome-div").textContent=`Welcome ${n}, you are now connected :)`):(document.getElementById("title").textContent="Hi",document.getElementById("welcome-div").style.display="block",document.getElementById("welcome-div").textContent="Sorry. Your credentials doesn't match.")})})}function I(){const s=document.getElementById("registerBtn");s==null||s.addEventListener("click",()=>{var i,n;if(document.getElementById("connexionBlock").style.display="none",document.getElementById("register-container"))return;const t=document.createElement("div");t.id="register-container",t.innerHTML=`
	...
	`,document.body.appendChild(t),(i=document.getElementById("backToLogin"))==null||i.addEventListener("click",()=>{var o;(o=document.getElementById("register-container"))==null||o.remove(),document.getElementById("connexionBlock").style.display="flex"}),(n=document.getElementById("create-account"))==null||n.addEventListener("click",()=>{var l;const o=document.getElementById("new-username").value,d=document.getElementById("new-password").value,c=document.getElementById("new-alias").value;if(o&&d&&c){const h=document.createElement("p");h.id="registration-created",h.innerHTML="Wonderful. You have created your account. You can connect to your account now.",(l=document.getElementById("registrationBlock"))==null||l.appendChild(h)}fetch("/api/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:o,password:d,alias:c})}).then(h=>h.json()).then(h=>console.log("Account created:",h)).catch(h=>console.error(h))})})}function A(){document.getElementById("btn").addEventListener("click",()=>{const e=document.getElementById("btn").value,t=document.getElementById("btn").value;fetch("/api/auth/info",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:t,alias:e})}).then(i=>i.json()).then(i=>console.log("You are in info page & data:",i))})}class S{static render(){return`
	...
	</div>`}static init(){A()}}class y{static navigate(e){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",e==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(e){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=P.render(),P.init();break;case"dashboard":new U().initialize();break;case"login":this.app.innerHTML=S.render(),S.init();break;case"info":this.app.innerHTML=L.render(),L.init();break}history.pushState({},"",e==="home"?"/":`/${e}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")}),window.addEventListener("popstate",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")})}}a(y,"app",document.getElementById("app"));y.init();window.addEventListener("load",()=>{const s=window.location.pathname;y.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":s.includes("info")?"info":"home"),C(),I(),A()});window.addEventListener("popstate",()=>{const s=window.location.pathname;y.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":s.includes("info")?"info":"home")});
