var A=Object.defineProperty;var B=(s,t,e)=>t in s?A(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var a=(s,t,e)=>B(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function e(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=e(n);fetch(n.href,r)}})();const w="https://localhost:4443/api/auth",u=class u{constructor(){a(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const t=localStorage.getItem("user");return t?JSON.parse(t):null}async login(t,e,i){var n,r,d;try{const c=await fetch(`${w}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:e})}),o=await c.json();if(!c.ok||o.status!=="success")throw new Error(o.error||o.message||"Login failed");if(!((r=(n=o.data)==null?void 0:n.user)!=null&&r.email)||!((d=o.data)!=null&&d.token))throw new Error("Invalid server response format");const y={email:o.data.user.email,token:o.data.token,alias:i||o.data.user.alias||o.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(c){throw console.error("Login error:",c),new Error(c instanceof Error?c.message:"An unknown error occurred")}}async register(t,e,i){var n,r,d;try{const c=await fetch(`${w}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:e,...i&&{alias:i}})}),o=await c.json();if(!c.ok||o.status!=="success")throw new Error(o.error||o.message||"Registration failed");if(!((r=(n=o.data)==null?void 0:n.user)!=null&&r.email)||!((d=o.data)!=null&&d.token))throw new Error("Invalid server response format");const y={email:o.data.user.email,token:o.data.token,alias:i||o.data.user.alias||o.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(c){throw console.error("Registration error:",c),new Error(c instanceof Error?c.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(t){const e=this.getCurrentUser();e&&(e.alias=t,this.saveUser(e))}onAuthStateChanged(t){this.listeners.push(t)}saveUser(t){localStorage.setItem("user",JSON.stringify(t)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(t=>t())}};a(u,"instance");let v=u;const h=v.getInstance(),f={render(){const s=h.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `}}const g=new H,b={renderGameCanvas:()=>g.render(),initGameCanvas:()=>g.init(),stop:()=>g.stop(),setBallSpeed:s=>g.setBallSpeed(s),handleKeyDown:s=>g.setKey(s,!0),handleKeyUp:s=>g.setKey(s,!1)},E={renderGameSettings(){return`
      <div class="space-y-4 p-4">
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const t=parseFloat(s.value);b.setBallSpeed(t)})}},D={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),b.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),b.handleKeyUp(s.key)})}};class M{constructor(){a(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const t=document.getElementById("auth-container");if(t){const e=h.getCurrentUser();t.innerHTML=e?f.renderProfile(e.email):f.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",t=>{const e=t.target;e.closest("#login-btn")?(this.isLoginView=!0,this.render()):e.closest("#register-btn")?(this.isLoginView=!1,this.render()):e.closest("#logout-btn")?(h.logout(),this.isLoginView=!0,this.render()):e.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async t=>{t.target.id==="auth-form"&&(t.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const t=document.getElementById("email"),e=document.getElementById("password"),i=document.getElementById("auth-error");if(!t||!e||!i)return;const n=t.value,r=e.value;try{this.isLoginView?await h.login(n,r):await h.register(n,r),this.render()}catch(d){i.textContent=d.message}}}const _=new M,P={currentUser:null,isLogin:!0,render(){return`
    ...
    `},renderAuth(){const s=h.getCurrentUser();return s?f.renderProfile(s.email):f.renderAuthForm(this.isLogin)},init(){this.currentUser=h.getCurrentUser(),b.initGameCanvas(),D.init(),_.init(),E.initSettings(),h.onAuthStateChanged(()=>{this.currentUser=h.getCurrentUser(),document.getElementById("auth-container").innerHTML=this.renderAuth()})}};class F{constructor(){a(this,"currentUser",null);this.initializeUser()}initializeUser(){const t=localStorage.getItem("authToken"),e=localStorage.getItem("userEmail");t&&e?this.currentUser={email:e,token:t}:m.navigate("home")}render(){var t;return`
	...
	</div>`}static init(){U()}}class m{static navigate(t){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",t==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(t){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=P.render(),P.init();break;case"dashboard":new F().initialize();break;case"login":this.app.innerHTML=S.render(),S.init();break;case"info":this.app.innerHTML=L.render(),L.init();break}history.pushState({},"",t==="home"?"/":`/${t}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const t=window.location.pathname;this.navigate(t.includes("dashboard")?"dashboard":t.includes("login")?"login":t.includes("info")?"info":"home")}),window.addEventListener("popstate",()=>{const t=window.location.pathname;this.navigate(t.includes("dashboard")?"dashboard":t.includes("login")?"login":t.includes("info")?"info":"home")})}}a(m,"app",document.getElementById("app"));m.init();window.addEventListener("load",()=>{const s=window.location.pathname;m.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":"home"),C(),I()});window.addEventListener("popstate",()=>{const s=window.location.pathname;m.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":"home")});
