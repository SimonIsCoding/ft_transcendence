var _=Object.defineProperty;var F=(s,e,t)=>e in s?_(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var a=(s,e,t)=>F(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();const S="https://localhost:4443/api/auth",u=class u{constructor(){a(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,i){var n,o,l;try{const d=await fetch(`${S}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Login failed");if(!((o=(n=c.data)==null?void 0:n.user)!=null&&o.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const p={email:c.data.user.email,token:c.data.token,alias:i||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(p),p}catch(d){throw console.error("Login error:",d),new Error(d instanceof Error?d.message:"An unknown error occurred")}}async register(e,t,i){var n,o,l;try{const d=await fetch(`${S}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...i&&{alias:i}}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Registration failed");if(!((o=(n=c.data)==null?void 0:n.user)!=null&&o.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const p={email:c.data.user.email,token:c.data.token,alias:i||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(p),p}catch(d){throw console.error("Registration error:",d),new Error(d instanceof Error?d.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};a(u,"instance");let E=u;const g=E.getInstance(),w={render(){const s=g.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `}}const m=new H;let I;function W(){I=new H}function j(){I&&I.renderMenuLayout()}const v={renderGameCanvas:()=>m.render(),initGameCanvas:()=>m.init(),stop:()=>m.stop(),setBallSpeed:s=>m.setBallSpeed(s),handleKeyDown:s=>m.setKey(s,!0),handleKeyUp:s=>m.setKey(s,!1)},C={renderGameSettings(){return`
      <div class="space-y-4 p-4">
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const e=parseFloat(s.value);v.setBallSpeed(e)})}},z={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),v.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),v.handleKeyUp(s.key)})}};class N{constructor(){a(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const e=document.getElementById("auth-container");if(e){const t=g.getCurrentUser();e.innerHTML=t?w.renderProfile(t.email):w.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",e=>{const t=e.target;t.closest("#loginBtn")?(this.isLoginView=!0,this.render()):t.closest("#register-btn")?(this.isLoginView=!1,this.render()):t.closest("#logout-btn")?(g.logout(),this.isLoginView=!0,this.render()):t.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async e=>{e.target.id==="auth-form"&&(e.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const e=document.getElementById("email"),t=document.getElementById("password"),i=document.getElementById("auth-error");if(!e||!t||!i)return;const n=e.value,o=t.value;try{this.isLoginView?await g.login(n,o):await g.register(n,o),this.render()}catch(l){i.textContent=l.message}}}const R=new N,B={renderMenu:()=>`
	<div class="flex items-center justify-center bg-[#fbd11b] p-2 w-full">
    ...
    `,init(){const s=document.getElementById("game-canvas");(s==null?void 0:s.getContext("2d"))?(W(),j()):console.error("Canvas non trouvé")}},k={currentUser:null,isLogin:!0,render(){return`
	...
	<main class="flex-1 flex items-center justify-center bg-[#fbd11b] overflow-y-auto min-h-[450px]">
      ...
      <footer class="bg-[#fbd11b] p-4">
        ${C.renderGameSettings()}
      </footer>

    </div>
    `},renderAuth(){const s=g.getCurrentUser();return s?w.renderProfile(s.email):w.renderAuthForm(this.isLogin)},init(){z.init(),R.init(),C.initSettings(),g.onAuthStateChanged(()=>{document.getElementById("auth-container").innerHTML=this.renderAuth()}),B.init()}};class V{constructor(){a(this,"currentUser",null);this.initializeUser()}initializeUser(){const e=localStorage.getItem("token"),t=localStorage.getItem("userEmail");e&&t?this.currentUser={email:t,token:e}:f.navigate("home")}render(){var e;return`
	...
	</div>`}static init(){D(),M(e=>{$(e)})}}class T{static render(){return`
	...
	</div>`}static init(){Y();const e=localStorage.getItem("login")||"",t=localStorage.getItem("alias")||"";console.log("current login in localStorage:",e),console.log("current alias in localStorage:",t);const i=document.getElementById("current-login"),n=document.getElementById("current-alias"),o=document.getElementById("new-login"),l=document.getElementById("new-alias");i&&(i.innerText=`Current login: ${e} - Change it to:`),n&&(n.innerText=`Current alias: ${t} - Change it to:`),o&&(o.value=e),l&&(l.value=t),G()}}class f{static navigate(e){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",e==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(e){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=k.render(),k.init();break;case"dashboard":new V().initialize();break;case"login":this.app.innerHTML=A.render(),A.init();break;case"info":this.app.innerHTML=T.render(),T.init();break}history.pushState({},"",e==="home"?"/":`/${e}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")}),window.addEventListener("popstate",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")})}}a(f,"app",document.getElementById("app"));f.init();window.addEventListener("load",()=>{const s=window.location.pathname;f.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":s.includes("info")?"info":"home"),D(),M(e=>{console.log("in app.ts, user",e)}),G()});window.addEventListener("popstate",()=>{const s=window.location.pathname;f.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":s.includes("info")?"info":"home")});
