var A=Object.defineProperty;var C=(s,e,t)=>e in s?A(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var a=(s,e,t)=>C(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function t(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=t(n);fetch(n.href,r)}})();const f="https://localhost:4443/api/auth",u=class u{constructor(){a(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,i){var n,r,d;try{const c=await fetch(`${f}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t})}),o=await c.json();if(!c.ok||o.status!=="success")throw new Error(o.error||o.message||"Login failed");if(!((r=(n=o.data)==null?void 0:n.user)!=null&&r.email)||!((d=o.data)!=null&&d.token))throw new Error("Invalid server response format");const y={email:o.data.user.email,token:o.data.token,alias:i||o.data.user.alias||o.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(c){throw console.error("Login error:",c),new Error(c instanceof Error?c.message:"An unknown error occurred")}}async register(e,t,i){var n,r,d;try{const c=await fetch(`${f}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...i&&{alias:i}})}),o=await c.json();if(!c.ok||o.status!=="success")throw new Error(o.error||o.message||"Registration failed");if(!((r=(n=o.data)==null?void 0:n.user)!=null&&r.email)||!((d=o.data)!=null&&d.token))throw new Error("Invalid server response format");const y={email:o.data.user.email,token:o.data.token,alias:i||o.data.user.alias||o.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(c){throw console.error("Registration error:",c),new Error(c instanceof Error?c.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};a(u,"instance");let v=u;const h=v.getInstance(),w={render(){const s=h.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `},renderAuth(){const s=h.getCurrentUser();return s?w.renderProfile(s.email):w.renderAuthForm(this.isLogin)},init(){this.currentUser=h.getCurrentUser(),b.initGameCanvas(),D.init(),T.init(),E.initSettings(),h.onAuthStateChanged(()=>{this.currentUser=h.getCurrentUser(),document.getElementById("auth-container").innerHTML=this.renderAuth()})}};class M{constructor(){a(this,"currentUser",null);this.initializeUser()}initializeUser(){const e=localStorage.getItem("authToken"),t=localStorage.getItem("userEmail");e&&t?this.currentUser={email:t,token:e}:m.navigate("home")}render(){var e;return`
	...
	`,document.body.appendChild(i),(n=document.getElementById("create-account"))==null||n.addEventListener("click",()=>{const r=document.getElementById("new-username").value,d=document.getElementById("new-password").value,c=document.getElementById("new-alias").value;fetch("/api/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:r,password:d,alias:c})}).then(o=>o.json()).then(o=>console.log("Account created:",o)).catch(o=>console.error(o))})}),document.addEventListener("DOMContentLoaded",()=>{var n,r;const t=document.getElementById("registrationBlock"),i=document.getElementById("connexionBlock");(n=document.getElementById("registerBtn"))==null||n.addEventListener("click",()=>{i.classList.add("hidden"),t.classList.remove("hidden")}),(r=document.getElementById("backToLogin"))==null||r.addEventListener("click",()=>{t.classList.add("hidden"),i.classList.remove("hidden")})})}class L{static render(){return`
	...
	</div>`}static init(){console.log("in webdev/pong/src/views/login.ts: Login page initialized"),S()}}class m{static navigate(e){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",e==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(e){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=P.render(),P.init();break;case"dashboard":new M().initialize();break;case"login":this.app.innerHTML=L.render(),L.init();break}history.pushState({},"",e==="home"?"/":`/${e}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":"home")}),window.addEventListener("popstate",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":"home")})}}a(m,"app",document.getElementById("app"));m.init();window.addEventListener("load",()=>{const s=window.location.pathname;m.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":"home"),S()});window.addEventListener("popstate",()=>{const s=window.location.pathname;m.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":"home")});
