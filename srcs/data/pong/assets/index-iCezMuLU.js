var D=Object.defineProperty;var M=(s,e,t)=>e in s?D(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var a=(s,e,t)=>M(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();const I="https://localhost:4443/api/auth",u=class u{constructor(){a(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,i){var n,o,l;try{const d=await fetch(`${I}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Login failed");if(!((o=(n=c.data)==null?void 0:n.user)!=null&&o.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const p={email:c.data.user.email,token:c.data.token,alias:i||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(p),p}catch(d){throw console.error("Login error:",d),new Error(d instanceof Error?d.message:"An unknown error occurred")}}async register(e,t,i){var n,o,l;try{const d=await fetch(`${I}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...i&&{alias:i}}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Registration failed");if(!((o=(n=c.data)==null?void 0:n.user)!=null&&o.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const p={email:c.data.user.email,token:c.data.token,alias:i||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(p),p}catch(d){throw console.error("Registration error:",d),new Error(d instanceof Error?d.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};a(u,"instance");let E=u;const g=E.getInstance(),w={render(){const s=g.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `}}const m=new G,v={renderGameCanvas:()=>m.render(),initGameCanvas:()=>m.init(),stop:()=>m.stop(),setBallSpeed:s=>m.setBallSpeed(s),handleKeyDown:s=>m.setKey(s,!0),handleKeyUp:s=>m.setKey(s,!1)},L={renderGameSettings(){return`
      <div class="space-y-4 p-4">
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const e=parseFloat(s.value);v.setBallSpeed(e)})}},U={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),v.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),v.handleKeyUp(s.key)})}};class O{constructor(){a(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const e=document.getElementById("auth-container");if(e){const t=g.getCurrentUser();e.innerHTML=t?w.renderProfile(t.email):w.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",e=>{const t=e.target;t.closest("#loginBtn")?(this.isLoginView=!0,this.render()):t.closest("#register-btn")?(this.isLoginView=!1,this.render()):t.closest("#logout-btn")?(g.logout(),this.isLoginView=!0,this.render()):t.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async e=>{e.target.id==="auth-form"&&(e.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const e=document.getElementById("email"),t=document.getElementById("password"),i=document.getElementById("auth-error");if(!e||!t||!i)return;const n=e.value,o=t.value;try{this.isLoginView?await g.login(n,o):await g.register(n,o),this.render()}catch(l){i.textContent=l.message}}}const W=new O,j={renderMenu:()=>`
    <div class="flex items-center justify-center bg-[#fbd11b] p-2 w-full">
      <div class="aspect-[4/3] w-full max-w-[1024px] min-w-[600px] bg-black border-4 border-white">
        <canvas 
          id="game-canvas" 
          class="w-full h-full"
        ></canvas>
		<div id="buttons" class="flex flex-col items-center justify-center min-h-screen">
	    <a href="/login" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded">Login</a>
        <a href="/register" class="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded">Register</a>
		</div>
      </div>
    </div>
  `},B={currentUser:null,isLogin:!0,render(){return`
    ...
    `},renderAuth(){const s=g.getCurrentUser();return s?w.renderProfile(s.email):w.renderAuthForm(this.isLogin)},init(){U.init(),W.init(),L.initSettings(),g.onAuthStateChanged(()=>{document.getElementById("auth-container").innerHTML=this.renderAuth()})}};class z{constructor(){a(this,"currentUser",null);this.initializeUser()}initializeUser(){const e=localStorage.getItem("token"),t=localStorage.getItem("userEmail");e&&t?this.currentUser={email:t,token:e}:f.navigate("home")}render(){var e;return`
	...
	</div>`}static init(){k(),T(e=>{N(e)})}}class A{static render(){return`
