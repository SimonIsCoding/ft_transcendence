var A=Object.defineProperty;var C=(s,t,e)=>t in s?A(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var n=(s,t,e)=>C(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function e(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(a){if(a.ep)return;a.ep=!0;const r=e(a);fetch(a.href,r)}})();const b="https://localhost:4443/api/auth",u=class u{constructor(){n(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const t=localStorage.getItem("user");return t?JSON.parse(t):null}async login(t,e,i){var a,r,d;try{const c=await fetch(`${b}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:e})}),o=await c.json();if(!c.ok||o.status!=="success")throw new Error(o.error||o.message||"Login failed");if(!((r=(a=o.data)==null?void 0:a.user)!=null&&r.email)||!((d=o.data)!=null&&d.token))throw new Error("Invalid server response format");const y={email:o.data.user.email,token:o.data.token,alias:i||o.data.user.alias||o.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(c){throw console.error("Login error:",c),new Error(c instanceof Error?c.message:"An unknown error occurred")}}async register(t,e,i){var a,r,d;try{const c=await fetch(`${b}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:e,...i&&{alias:i}})}),o=await c.json();if(!c.ok||o.status!=="success")throw new Error(o.error||o.message||"Registration failed");if(!((r=(a=o.data)==null?void 0:a.user)!=null&&r.email)||!((d=o.data)!=null&&d.token))throw new Error("Invalid server response format");const y={email:o.data.user.email,token:o.data.token,alias:i||o.data.user.alias||o.data.user.email.split("@")[0]};return this.saveUser(y),y}catch(c){throw console.error("Registration error:",c),new Error(c instanceof Error?c.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(t){const e=this.getCurrentUser();e&&(e.alias=t,this.saveUser(e))}onAuthStateChanged(t){this.listeners.push(t)}saveUser(t){localStorage.setItem("user",JSON.stringify(t)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(t=>t())}};n(u,"instance");let f=u;const h=f.getInstance(),v={render(){const s=h.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const t=parseFloat(s.value);w.setBallSpeed(t)})}},D={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),w.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),w.handleKeyUp(s.key)})}};class H{constructor(){n(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const t=document.getElementById("auth-container");if(t){const e=h.getCurrentUser();t.innerHTML=e?v.renderProfile(e.email):v.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",t=>{const e=t.target;e.closest("#login-btn")?(this.isLoginView=!0,this.render()):e.closest("#register-btn")?(this.isLoginView=!1,this.render()):e.closest("#logout-btn")?(h.logout(),this.isLoginView=!0,this.render()):e.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async t=>{t.target.id==="auth-form"&&(t.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const t=document.getElementById("email"),e=document.getElementById("password"),i=document.getElementById("auth-error");if(!t||!e||!i)return;const a=t.value,r=e.value;try{this.isLoginView?await h.login(a,r):await h.register(a,r),this.render()}catch(d){i.textContent=d.message}}}const T=new H,P={currentUser:null,isLogin:!0,render(){return`
    ...
    `},renderAuth(){const s=h.getCurrentUser();return s?v.renderProfile(s.email):v.renderAuthForm(this.isLogin)},init(){this.currentUser=h.getCurrentUser(),w.initGameCanvas(),D.init(),T.init(),E.initSettings(),h.onAuthStateChanged(()=>{this.currentUser=h.getCurrentUser(),document.getElementById("auth-container").innerHTML=this.renderAuth()})}};class M{constructor(){n(this,"currentUser",null);this.initializeUser()}initializeUser(){const t=localStorage.getItem("authToken"),e=localStorage.getItem("userEmail");t&&e?this.currentUser={email:e,token:t}:m.navigate("home")}render(){var t;return`
	...
	`,document.body.appendChild(i),(a=document.getElementById("create-account"))==null||a.addEventListener("click",()=>{const r=document.getElementById("new-username").value,d=document.getElementById("new-password").value,c=document.getElementById("new-alias").value;fetch("/api/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:r,password:d,alias:c})}).then(o=>o.json()).then(o=>console.log("Account created:",o)).catch(o=>console.error(o))})})}class S{static render(){return`<div class="flex flex-col items-center justify-center min-h-screen bg-gray-100">
	...
	<div class="mb-4 px-4 py-2 border rounded w-64 flex justify-between">
