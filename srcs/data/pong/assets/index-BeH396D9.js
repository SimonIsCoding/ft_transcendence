var M=Object.defineProperty;var G=(s,e,t)=>e in s?M(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var a=(s,e,t)=>G(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();const P="https://localhost:4443/api/auth",u=class u{constructor(){a(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,i){var n,o,l;try{const d=await fetch(`${P}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Login failed");if(!((o=(n=c.data)==null?void 0:n.user)!=null&&o.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const m={email:c.data.user.email,token:c.data.token,alias:i||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(m),m}catch(d){throw console.error("Login error:",d),new Error(d instanceof Error?d.message:"An unknown error occurred")}}async register(e,t,i){var n,o,l;try{const d=await fetch(`${P}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...i&&{alias:i}}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Registration failed");if(!((o=(n=c.data)==null?void 0:n.user)!=null&&o.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const m={email:c.data.user.email,token:c.data.token,alias:i||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(m),m}catch(d){throw console.error("Registration error:",d),new Error(d instanceof Error?d.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};a(u,"instance");let E=u;const g=E.getInstance(),w={render(){const s=g.getCurrentUser();return s?this.renderProfile(s.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `}}const p=new U,v={renderGameCanvas:()=>p.render(),initGameCanvas:()=>p.init(),stop:()=>p.stop(),setBallSpeed:s=>p.setBallSpeed(s),handleKeyDown:s=>p.setKey(s,!0),handleKeyUp:s=>p.setKey(s,!1)},L={renderGameSettings(){return`
      <div class="space-y-4 p-4">
    ...
    `},initSettings(){const s=document.getElementById("ball-speed");s&&s.addEventListener("input",()=>{const e=parseFloat(s.value);v.setBallSpeed(e)})}},O={init(){window.addEventListener("keydown",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),v.handleKeyDown(s.key)}),window.addEventListener("keyup",s=>{["ArrowUp","ArrowDown","w","s"].includes(s.key)&&s.preventDefault(),v.handleKeyUp(s.key)})}};class W{constructor(){a(this,"isLoginView",!0)}init(){this.render(),this.bindAuthEvents()}render(){const e=document.getElementById("auth-container");if(e){const t=g.getCurrentUser();e.innerHTML=t?w.renderProfile(t.email):w.renderAuthForm(this.isLoginView)}}bindAuthEvents(){document.addEventListener("click",e=>{const t=e.target;t.closest("#loginBtn")?(this.isLoginView=!0,this.render()):t.closest("#register-btn")?(this.isLoginView=!1,this.render()):t.closest("#logout-btn")?(g.logout(),this.isLoginView=!0,this.render()):t.closest("#toggle-auth")&&(this.isLoginView=!this.isLoginView,this.render())}),document.addEventListener("submit",async e=>{e.target.id==="auth-form"&&(e.preventDefault(),await this.handleAuthFormSubmit())})}async handleAuthFormSubmit(){const e=document.getElementById("email"),t=document.getElementById("password"),i=document.getElementById("auth-error");if(!e||!t||!i)return;const n=e.value,o=t.value;try{this.isLoginView?await g.login(n,o):await g.register(n,o),this.render()}catch(l){i.textContent=l.message}}}const j=new W;function z(s,e){e.strokeStyle="rgba(255, 255, 255, 0.5)",e.lineWidth=4,e.setLineDash([20,15]),e.beginPath(),e.moveTo(s.width/2,0),e.lineTo(s.width/2,s.height),e.stroke(),e.setLineDash([])}const C={renderMenu:()=>`
	<div class="flex items-center justify-center bg-[#fbd11b] p-2 w-full">
    ...
    `,init(){const s=document.getElementById("game-canvas");if(!s)return;const e=s.getContext("2d");e&&z(s,e)}},B={currentUser:null,isLogin:!0,render(){return`
	...
	</div>`}static init(){T(),D(e=>{V(e)})}}class A{static render(){return`
	...
	</div>`}static init(){$();const e=localStorage.getItem("login")||"",t=localStorage.getItem("alias")||"";console.log("current login in localStorage:",e),console.log("current alias in localStorage:",t);const i=document.getElementById("current-login"),n=document.getElementById("current-alias"),o=document.getElementById("new-login"),l=document.getElementById("new-alias");i&&(i.innerText=`Current login: ${e} - Change it to:`),n&&(n.innerText=`Current alias: ${t} - Change it to:`),o&&(o.value=e),l&&(l.value=t),H()}}class f{static navigate(e){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",e==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(e){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=B.render(),B.init();break;case"dashboard":new N().initialize();break;case"login":this.app.innerHTML=k.render(),k.init();break;case"info":this.app.innerHTML=A.render(),A.init();break}history.pushState({},"",e==="home"?"/":`/${e}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")}),window.addEventListener("popstate",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")})}}a(f,"app",document.getElementById("app"));f.init();window.addEventListener("load",()=>{const s=window.location.pathname;f.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":s.includes("info")?"info":"home"),T(),D(e=>{console.log("in app.ts, user",e)}),H()});window.addEventListener("popstate",()=>{const s=window.location.pathname;f.navigate(s.includes("dashboard")?"dashboard":s.includes("login")?"login":s.includes("info")?"info":"home")});
