var D=Object.defineProperty;var M=(i,e,t)=>e in i?D(i,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):i[e]=t;var o=(i,e,t)=>M(i,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(n){if(n.ep)return;n.ep=!0;const a=t(n);fetch(n.href,a)}})();const S="https://localhost:4443/api/auth",u=class u{constructor(){o(this,"listeners",[])}static getInstance(){return u.instance||(u.instance=new u),u.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,s){var n,a,l;try{const d=await fetch(`${S}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Login failed");if(!((a=(n=c.data)==null?void 0:n.user)!=null&&a.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const m={email:c.data.user.email,token:c.data.token,alias:s||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(m),m}catch(d){throw console.error("Login error:",d),new Error(d instanceof Error?d.message:"An unknown error occurred")}}async register(e,t,s){var n,a,l;try{const d=await fetch(`${S}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...s&&{alias:s}}),credentials:"include"}),c=await d.json();if(!d.ok||c.status!=="success")throw new Error(c.error||c.message||"Registration failed");if(!((a=(n=c.data)==null?void 0:n.user)!=null&&a.email)||!((l=c.data)!=null&&l.token))throw new Error("Invalid server response format");const m={email:c.data.user.email,token:c.data.token,alias:s||c.data.user.alias||c.data.user.email.split("@")[0]};return this.saveUser(m),m}catch(d){throw console.error("Registration error:",d),new Error(d instanceof Error?d.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};o(u,"instance");let E=u;const g=E.getInstance(),v={render(){const i=g.getCurrentUser();return i?this.renderProfile(i.email):this.renderAuthSelector()},renderAuthSelector(){return`
    ...
    `}}const p=new F,w={renderGameCanvas:()=>p.render(),initGameCanvas:()=>p.init(),stop:()=>p.stop(),setBallSpeed:i=>p.setBallSpeed(i),handleKeyDown:i=>p.setKey(i,!0),handleKeyUp:i=>p.setKey(i,!1)},B={renderGameSettings(){return`
      <div class="space-y-4 p-4">
	...
	</div>`}static init(){T(),P(e=>{console.log("in LoginView.init(), user: ",e)})}}class k{static render(){return`
