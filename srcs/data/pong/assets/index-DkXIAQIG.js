var k=Object.defineProperty;var T=(i,e,t)=>e in i?k(i,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):i[e]=t;var o=(i,e,t)=>T(i,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const c of a.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&s(c)}).observe(document,{childList:!0,subtree:!0});function t(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(n){if(n.ep)return;n.ep=!0;const a=t(n);fetch(n.href,a)}})();const x="https://localhost:4443/api/auth",m=class m{constructor(){o(this,"listeners",[])}static getInstance(){return m.instance||(m.instance=new m),m.instance}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}async login(e,t,s){var n,a,c;try{const d=await fetch(`${x}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t}),credentials:"include"}),r=await d.json();if(!d.ok||r.status!=="success")throw new Error(r.error||r.message||"Login failed");if(!((a=(n=r.data)==null?void 0:n.user)!=null&&a.email)||!((c=r.data)!=null&&c.token))throw new Error("Invalid server response format");const h={email:r.data.user.email,token:r.data.token,alias:s||r.data.user.alias||r.data.user.email.split("@")[0]};return this.saveUser(h),h}catch(d){throw console.error("Login error:",d),new Error(d instanceof Error?d.message:"An unknown error occurred")}}async register(e,t,s){var n,a,c;try{const d=await fetch(`${x}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t,...s&&{alias:s}}),credentials:"include"}),r=await d.json();if(!d.ok||r.status!=="success")throw new Error(r.error||r.message||"Registration failed");if(!((a=(n=r.data)==null?void 0:n.user)!=null&&a.email)||!((c=r.data)!=null&&c.token))throw new Error("Invalid server response format");const h={email:r.data.user.email,token:r.data.token,alias:s||r.data.user.alias||r.data.user.email.split("@")[0]};return this.saveUser(h),h}catch(d){throw console.error("Registration error:",d),new Error(d instanceof Error?d.message:"Registration failed")}}logout(){localStorage.removeItem("user"),this.notifyListeners()}updateAlias(e){const t=this.getCurrentUser();t&&(t.alias=e,this.saveUser(t))}onAuthStateChanged(e){this.listeners.push(e)}saveUser(e){localStorage.setItem("user",JSON.stringify(e)),this.notifyListeners()}notifyListeners(){this.listeners.forEach(e=>e())}};o(m,"instance");let v=m;const u=v.getInstance(),w={render(){const i=u.getCurrentUser();return i?this.renderProfile(i.email):this.renderAuthSelector()},renderAuthSelector(){return`
	...
	`,document.body.appendChild(t),(s=document.getElementById("backToLogin"))==null||s.addEventListener("click",()=>{var a;(a=document.getElementById("register-container"))==null||a.remove(),document.getElementById("connexionBlock").style.display="flex"}),(n=document.getElementById("create-account"))==null||n.addEventListener("click",()=>{const a=document.getElementById("new-username").value,c=document.getElementById("new-password").value,d=document.getElementById("new-alias").value;fetch("/api/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:a,password:c,alias:d}),credentials:"include"}).then(r=>{const h=document.getElementById("registrationBlock");let g=document.getElementById("registration-created");return g||(g=document.createElement("p"),g.id="registration-created",h==null||h.appendChild(g)),r.status===200?g.textContent="Wonderful. You have created your account. You can connect to your account now.":r.status===500?g.textContent="DB Error":g.textContent="An account with this login has already been created.",r.json()}).then(r=>{console.log("Account created:",r)}).catch(r=>console.error(r))})})}function A(){fetch("/api/auth/info",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({}),credentials:"include"}).then(i=>i.json()).then(i=>{console.log("You are in info page & data:",i)})}async function O(){try{const i=await fetch("/api/auth/info",{method:"GET",credentials:"include"});if(!i.ok)throw new Error("Not authenticated");const e=await i.json();return console.log("User info received:",e),!0}catch(i){return console.warn("User not authenticated:",i),!1}}const I={currentUser:null,isLogin:!0,render(){return`
    ...
    `},renderAuth(){const i=u.getCurrentUser();return i?w.renderProfile(i.email):w.renderAuthForm(this.isLogin)},init(){this.currentUser=u.getCurrentUser(),b.initGameCanvas(),U.init(),G.init(),P.initSettings(),u.onAuthStateChanged(()=>{this.currentUser=u.getCurrentUser(),document.getElementById("auth-container").innerHTML=this.renderAuth()}),F()}};class W{constructor(){o(this,"currentUser",null);this.initializeUser()}initializeUser(){const e=localStorage.getItem("token"),t=localStorage.getItem("userEmail");e&&t?this.currentUser={email:t,token:e}:f.navigate("home")}render(){var e;return`
	...
	</div>`}static init(){L(),B()}}class C{static render(){return`
	...
	</div>`}static init(){O();const e=localStorage.getItem("login")||"",t=localStorage.getItem("alias")||"";console.log("current login in localStorage:",e),console.log("current alias in localStorage:",t);const s=document.getElementById("current-login"),n=document.getElementById("current-alias"),a=document.getElementById("new-login"),c=document.getElementById("new-alias");s&&(s.innerText=`Current login: ${e} - Change it to:`),n&&(n.innerText=`Current alias: ${t} - Change it to:`),a&&(a.value=e),c&&(c.value=t),A()}}class f{static navigate(e){if(!this.app){console.error("App container not found");return}if(this.app.innerHTML="",e==="dashboard"&&!this.isAuthenticated())return console.warn("Redirecting to home: not authenticated"),this.navigate("home");switch(e){case"home":if(this.isAuthenticated())return this.navigate("dashboard");this.app.innerHTML=I.render(),I.init();break;case"dashboard":new W().initialize();break;case"login":this.app.innerHTML=S.render(),S.init();break;case"info":this.app.innerHTML=C.render(),C.init();break}history.pushState({},"",e==="home"?"/":`/${e}`)}static isAuthenticated(){return localStorage.getItem("authToken")!==null}static init(){window.addEventListener("load",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")}),window.addEventListener("popstate",()=>{const e=window.location.pathname;this.navigate(e.includes("dashboard")?"dashboard":e.includes("login")?"login":e.includes("info")?"info":"home")})}}o(f,"app",document.getElementById("app"));f.init();window.addEventListener("load",()=>{const i=window.location.pathname;f.navigate(i.includes("dashboard")?"dashboard":i.includes("login")?"login":i.includes("info")?"info":"home"),L(),B(),A()});window.addEventListener("popstate",()=>{const i=window.location.pathname;f.navigate(i.includes("dashboard")?"dashboard":i.includes("login")?"login":i.includes("info")?"info":"home")});
