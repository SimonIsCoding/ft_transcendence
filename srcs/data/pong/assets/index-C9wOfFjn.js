...
winner is===`,i),s&&e.winner){s.innerHTML=`            
	...
	`},async init(){const t=document.getElementById("playBtn");t==null||t.addEventListener("click",()=>{v.gameType="p-vs-p",v.inGame=!0,w.navigate("game")})}};function je(){return`
	...
	`},init(){bt()}},Ie={currentUser:null,isLogin:!0,render(){return`
  ...
  `},async init(){await O(),se.init(),ie.init();const t=document.getElementById("gameArea"),e=document.getElementById("oneVsOneArea"),n=document.getElementById("oneVsAIArea"),s=document.getElementById("player1"),i=document.getElementById("player2"),o=document.getElementById("player1VSAI");let a=i;console.log(v.gameType),t==null||t.classList.add("hidden"),v.gameType==="p-vs-p"?(e==null||e.classList.remove("hidden"),n==null||n.classList.add("hidden")):v.gameType==="p-vs-ai"&&(a=o,v.otherPlayer=a.value,e==null||e.classList.add("hidden"),n==null||n.classList.remove("hidden")),!a&&!s&&new v().initGame({player1:{alias:"User 1"},player2:{alias:"User 2"},winner:null})}};class w{static navigate(e,n=!0){if(!this.app){console.error("App container not found");return}switch(e){case"home":v.cleanup(),v.gameController=!1,this.app.innerHTML=Ie.render(),Ie.init();break;case"login":v.cleanup(),this.app.innerHTML=Be.render(),Be.init();break;case"register":v.cleanup(),this.app.innerHTML=ke.render(),ke.init();break;case"game":console.log("game"),v.inGame=!0,v.gameType="p-vs-p",this.app.innerHTML=W.render(),W.init();break;case"gameai":console.log("gameai"),v.inGame=!0,v.gameType="p-vs-ai",this.app.innerHTML=W.render(),W.init();break;case"tournament":console.log("router"),this.app.innerHTML=De.render(),De.init();break}n&&history.pushState({},"",e==="home"?"/":`/${e}`)}static resolveRoute(e){return e.includes("login")?"login":e.includes("register")?"register":e.includes("gameai")?"gameai":e.includes("game")?"game":e.includes("tournament")?"tournament":"home"}static init(){window.addEventListener("load",()=>{const e=this.resolveRoute(window.location.pathname);this.navigate(e,!1)}),window.addEventListener("popstate",()=>{const e=this.resolveRoute(window.location.pathname);this.navigate(e,!1)}),window.addEventListener("popstate",e=>{console.log("History changed:",e.state)})}}l(w,"app",document.getElementById("app")),l(w,"currentUser");w.init();window.addEventListener("load",()=>{const t=window.location.pathname;w.navigate(t.includes("login")?"login":t.includes("register")?"register":t.includes("gameai")||t.includes("game")?"game":t.includes("tournament")?"tournament":"home",!1)});window.addEventListener("popstate",()=>{const t=window.location.pathname;w.navigate(t.includes("login")?"login":t.includes("register")?"register":t.includes("gameai")||t.includes("game")?"game":t.includes("tournament")?"tournament":"home",!1)});
