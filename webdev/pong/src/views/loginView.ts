...
export const loginView = {
  render: (): string => `
	<div id="loginCredentials" class="flex flex-col justify-center items-center w-full space-y-10">
		<input id="login" type="text" placeholder="Login" class="text-white px-6 py-3 text-5xl border border-white rounded w-120" />
		<div class="relative">
			<input id="password" type="password" placeholder="Password" class="text-white px-6 py-3 text-5xl border border-white rounded w-120" />
		...
		<button id="connectionBtn" class="text-white px-6 py-3 text-5xl border border-white rounded w-120">Log in</button>
		<button id="backToRegister" class="text-white px-2 py-1 text-2xl underline">Click here to create an account</button>
	</div>
  `,
