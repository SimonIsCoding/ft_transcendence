...
export const friendsCard = {
	render(friendUser: User): string {
	return `
	<div id="friendBox_${friendUser.id}" class="flex flex-col rounded-2xl w-full space-y-5 shadow-base shadow-gray-600 pr-5 pl-5 pt-2 pb-2 bg-black mb-2">
		<div class="flex items-center space-x-2">
...
export const othersUsersCard = {
	render(name: string, userId: string): string {
	return `
	<div id="${name}" class="flex flex-col rounded-2xl w-full space-y-1 shadow-base shadow-gray-600 px-5 py-2 bg-black mb-3 opacity-100 transition-opacity">
		<div class="flex items-center space-x-2">
			<img id="othersUsersPhoto_${userId}" class="w-10 h-10 rounded-full object-cover border border-black bg-[#fbd11b] text-black flex items-center justify-center text-xl font-bold group-hover:bg-black group-hover:text-[#fbd11b] transition shadow-md" />
			
			<div class="flex flex-col justify-center">
				<p id="othersUsersUsername_${userId}" class="font-bold text-sm text-[#fbd11b]">FriendUsername</p>
			</div>
		</div>

		<div class="flex justify-center space-x-2">
			<button id="addFriendBtn_${userId}" class="px-2 py-1 rounded-full border border-[#fbd11b] text-[#fbd11b] bg-black hover:bg-[#fbd11b] hover:text-black transition font-bold text-sm w-full">
				Add as a friend
			</button>
		</div>
	</div>
